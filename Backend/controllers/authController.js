...
const editUser = async (req, res) => {
  try {
    ...
    let hashedPassword = currentUser.password; // Keep existing password by default

    // If plain_password is provided, hash it and update the main password
    if (plain_password && plain_password.trim() !== '') {
      ...
      const salt = await bcrypt.genSalt(10);
      hashedPassword = await bcrypt.hash(plain_password, salt);
    }
    ...
    const updateData = {
      ...
      expertise: expertise || currentUser.expertise,
      address: address || currentUser.address,
      position: position || currentUser.position,
      description: description || currentUser.description,
      plain_password: plain_password || currentUser.plain_password,
    ...
    const result = await User.update(
      id, 
      updateData.username, 
      hashedPassword, // This will be the newly hashed password or existing one
    ...
    return res.json({ 
      ...
      passwordUpdated: plain_password && plain_password.trim() !== '' ? true : false
    });
