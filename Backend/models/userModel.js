...
// Update user
update: (id, username, hashedPassword, full_name, role, phone, gender, expertise, address, position, description, plain_password, image) => {
  return new Promise((resolve, reject) => {
    const sql = 'UPDATE users SET username = ?, password = ?, full_name = ?, role = ?, phone = ?, gender = ?, expertise = ?, address = ?, position = ?, description = ?, plain_password = ?, image = ?, updated_at = NOW() WHERE id = ?';
    pool.query(sql, [username, hashedPassword, full_name, role, phone, gender, expertise, address, position, description, plain_password, image, id], (err, result) => {
      if (err) return reject(err);
      if (result.affectedRows === 0) {
        return reject(new Error('User not found'));
      }
